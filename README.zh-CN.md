...
所有写操作通过 `--data` 或 `--data-file` 接收 JSON，查询支持 `--query` 或 `--query-file`；Agent 应使用 `--output json`。PowerShell 中优先使用 JSON 文件，避免原生命令参数转义差异：
...
| `workshop gui` | 必要时先后台启动服务，再在默认浏览器打开工作台 |
...
| `workshop update` | 执行 `npm update --global openworkshop` |
