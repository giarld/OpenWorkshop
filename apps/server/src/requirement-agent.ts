...
function createSession(config: Readonly<CodexRoleConfig>, configKey: string): RequirementSession {
  ...
  const client = CodexAppServer.launch({ ...(customArgs.length ? { args: [...CODEX_APP_SERVER_ARGS, ...customArgs] } : {}), onEvent: (event) => {
