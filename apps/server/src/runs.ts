...
const CONTROL_RUN_INSTRUCTIONS = "This is a bounded OpenWorkshop control turn. The complete decision context is included in the user prompt. Do not invoke tools, shell commands, MCP servers, Skills, plugins, hooks, or memory workflows. Do not inspect the workspace or read files. Analyze only the included context and return only the requested JSON.";
...
export class AgentRunController {
  ...
  async start(runId: string, cwd: string): Promise<void> {
    ...
    const controlRun = run.role === "supervisor" && ["coordinate", "plan_revision", "plan_revision_review"].includes(run.trigger_type);
    const controlContext = controlRun ? Object.entries(contextFiles).map(([name, content]) => `## ${name}\n\n${content}`).join("\n\n") : "";
    ...
    try {
      ...
      const rolePrompt = run.role === "reviewer" ? prompt + "\n\n" + REVIEWER_REWORK_CONTRACT : prompt;
      const effectivePrompt = controlRun ? CONTROL_RUN_INSTRUCTIONS + "\n\n" + rolePrompt : rolePrompt;
      const turnOptions: AgentStartOptions = {
        cwd,
        sandbox: sandboxMode,
        approvalPolicy,
        prompt: effectivePrompt,
        ...(controlRun ? { developerInstructions: CONTROL_RUN_INSTRUCTIONS } : {}),
