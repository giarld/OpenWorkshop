...
import { browserCommand, ensureBackgroundService, waitForServiceStop } from "./service-control.ts";
...
test("starts or reuses a detached service and observes when it stops", async () => {
  ...
  try {
    ...
    const state = await ensureBackgroundService(root, cli, "127.0.0.1", 9876);
    pid = state.pid;
    assert.equal(state.port, 9876);
    assert.equal((await ensureBackgroundService(root, undefined, "127.0.0.1", 9876)).pid, pid);
